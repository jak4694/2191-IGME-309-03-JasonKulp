...
void Application::Display(void)
{
	...
	//your code goes here
	//Updates the start/end nodes every second
	if(fTimer >= 1)
	{
		fTimer -= 1;
		currentIndex = (currentIndex + 1) % m_stopsList.size();
		nextIndex = (currentIndex + 1) % m_stopsList.size();
	}
	//Lerps between the current and next nodes given the time between them
	v3CurrentPos = glm::lerp(m_stopsList[currentIndex], m_stopsList[nextIndex], fTimer);
	//-------------------
