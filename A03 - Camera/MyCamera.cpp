...
{
	forwardVector = glm::normalize(m_v3Target - m_v3Position);
	rightVector = glm::normalize(glm::cross(AXIS_Y, forwardVector));
	glm::quat rotationQuat = glm::angleAxis(angle, rightVector);
	vector3 updatedForwardVector = (rotationQuat * forwardVector) * glm::conjugate(rotationQuat);
	//Sets the appropriate target based on the calculations
	SetTarget(m_v3Position + updatedForwardVector);
}
...
{
	forwardVector = glm::normalize(m_v3Target - m_v3Position);
	glm::quat rotationQuat = glm::angleAxis(angle, AXIS_Y);
	vector3 updatedForwardVector = (rotationQuat * forwardVector) * glm::conjugate(rotationQuat);
	//Sets the appropriate target based on the calculations
	SetTarget(m_v3Position + updatedForwardVector);
}

//Moves the camera forward or backwards based on the camera direction
void MyCamera::MoveForward(float a_fDistance)
{
	forwardVector = glm::normalize(m_v3Target - m_v3Position);
	m_v3Position += forwardVector * a_fDistance;
	m_v3Target += forwardVector * a_fDistance;
	m_v3Above += forwardVector * a_fDistance;
}

//Moves the camera straight up or straight down
...
//Moves the camera left or right based on the camera direction
void MyCamera::MoveSideways(float a_fDistance)
{
	...
	m_v3Above += rightVector * a_fDistance;
}
